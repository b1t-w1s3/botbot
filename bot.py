...
ከታች በተዘረዘሩት የሂሳብ ቁጥሮች 100 ብር እና ከዛም 
በላይ በማስገባት አገልግሎቱን በመደገፍ የልቤ ጌታ 
የተሰኘውን የዘማሪት ህሊና ዳዊትን ሙሉ አልበም ማግኘት 
ይችላሉ!
...
try:
  ...
  def sendMessageToCustomer():
    bot.send_message(Admin_telegram_ID, "Enter channel link")
    @bot.message_handler(content_types=['text'])
    def smtc(message):
      channel_link = message.text
      bot.reply_to(message, "Link recieved!")
    
      link_btn = types.InlineKeyboardButton('🔗Channel link',  url=channel_link)
      keyboard = types.InlineKeyboardMarkup()
      keyboard.add(link_btn)
      bot.send_message(userID, text=verified_message, reply_markup=keyboard)
...
📝 Request title: 💰 Subscription plans » Buy የልቤ ጌታ Full Album
📅 Request date: {}
📎 Request information
